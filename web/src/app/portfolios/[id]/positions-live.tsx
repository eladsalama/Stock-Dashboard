...
interface Props {
  portfolioId: string;
  initial: Position[];
  onMetrics?: (m: PositionsMetrics) => void;
}
export interface PositionsLiveHandle {
  startNew: () => void;
}
...
const PositionsLive = React.forwardRef<PositionsLiveHandle, Props>(function PositionsLive(
  { portfolioId, initial, onMetrics },
  ref,
) {
      ...
      try {
        ...
        setPositions((ps) =>
          ps.map((p) => {
            ...
            const dayChange =
              previousClose != null ? (price - previousClose) * Number(p.quantity) : undefined;
            const dayChangePct =
  ...
  function enterEdit(id: string) {
    setPositions((ps) =>
      ps.map((p) =>
        p.id === id ? { ...p, _editing: true } : p._editing ? { ...p, _editing: false } : p,
      ),
    );
  }
  function editField(id: string, field: "quantity" | "avgCost", value: number) {
  ...
  async function saveAndExit(id: string) {
    ...
    try {
      ...
      setPositions((ps) => ps.map((p) => (p.id === id ? { ...p, _editing: false } : p)));
    }
  }
  ...
  function updateNew(field: "symbol" | "quantity" | "avgCost", value: string) {
    setNewRow((r) =>
      r ? { ...r, [field]: field === "symbol" ? value.toUpperCase() : Number(value) } : r,
    );
  }
  function cancelNew() {
    setNewRow(null);
  ...
  async function saveNew() {
    ...
    if (!symbol || !isFinite(newRow.quantity) || newRow.quantity <= 0) {
      cancelNew();
      return;
    }
  ...
  const totalPrev = positions.reduce(
    (acc, p) => acc + (p.previousClose ?? p.price ?? 0) * Number(p.quantity),
    0,
  );
  ...
  const columns: Column<Enriched>[] = [
    {
      ...
      render: (r) =>
        r._new ? (
          ...
          <SymbolLink
            symbol={r.symbol}
            style={{ textDecoration: "none", color: "var(--color-accent)", cursor: "pointer" }}
          >
            {r.symbol}
          </SymbolLink>
        ),
          ...
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                ...
                setPositions((ps) =>
                  ps.map((p) => (p.id === r.id ? { ...p, _editing: false } : p)),
                );
              }
            }}
          />
    ...
    },
    {
      ...
      sortable: true,
      align: "right",
      render: (r) => (r.price ? r.price.toFixed(2) : r.error ? "Err" : "…"),
    },
    {
      key: "dayChange",
      label: "Day Δ",
      ...
      render: (r) =>
        r.dayChange !== undefined ? (
          <span className={r.dayChange >= 0 ? "pl-pos" : "pl-neg"}>{r.dayChange.toFixed(2)}</span>
        ) : (
          "…"
        ),
    },
    ...
    },
    {
      ...
      sortable: true,
      align: "right",
      render: (r) => (r.marketValue !== undefined ? r.marketValue.toFixed(2) : "…"),
    },
    {
      key: "pl",
      label: "Unreal. P/L",
      ...
      render: (r) =>
        r._new ? (
          <div style={{ display: "flex", gap: 4 }}>
            ...
            <button className="mini-btn" style={{ color: "#f85149" }} onClick={cancelNew}>
              ✕
            </button>
          </div>
        ) : (
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      <div style={{ fontSize: 10, opacity: 0.5 }}>
        Double‑click Qty / Avg Cost to edit. Use ＋ to add a new position (Avg Cost defaults to
        market price if left blank).
      </div>
    </div>
  );
});
