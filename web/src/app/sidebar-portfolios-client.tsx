...
function debugLog(label: string, payload: Record<string, unknown>) {
  ...
  try {
    ...
    try {
      console.warn("[sidebar] log exception", e);
    } catch {}
  }
}
...
export default function SidebarPortfolios() {
  ...
  const initialCache: Portfolio[] = (() => {
    ...
    // If not authenticated, never hydrate cached portfolios
    if (!token) return [];
    try {
      const raw = localStorage.getItem(LS_CACHE_KEY);
      if (raw) {
        const parsed = JSON.parse(raw);
        debugLog("hydrate-from-localStorage", { count: Array.isArray(parsed) ? parsed.length : 0 });
        return parsed;
      }
    } catch {}
    return [];
  ...
  const [selectedId, setSelectedId] = useState<string | null>(
    () =>
      selectedFromPath ||
      (typeof window !== "undefined" ? localStorage.getItem(LS_SELECTED_KEY) : null),
  );
  useEffect(() => {
    if (selectedFromPath) setSelectedId(selectedFromPath);
  }, [selectedFromPath]);
  useEffect(() => {
    if (selectedId) localStorage.setItem(LS_SELECTED_KEY, selectedId);
  }, [selectedId]);
  ...
  const applyList = useCallback(
    (list: Portfolio[], phaseOverride?: Phase, meta?: Record<string, unknown>) => {
      ...
      debugLog("applyList", {
        ...
        prev: dataRef.current.length,
        phaseOverride,
        meta,
      });
      // Update React state (do not force "loading" once we have data)
      setState((s) => ({
        phase: phaseOverride || (s.phase === "idle" ? "ready" : s.phase),
        data: list,
      ...
      try {
        if (typeof window !== "undefined") {
          // @ts-expect-error diagnostic state snapshot
          window.__SIDEBAR_STATE = {
            phase: phaseOverride || state.phase,
            list: list.map((p) => p.id),
            selected: selectedId,
            ts: Date.now(),
          };
        }
  ...
  const fetchPortfolios = useCallback(async (attempt = 1): Promise<Portfolio[] | null> => {
    ...
    try {
      ...
      debugLog("fetchPortfolios:resp", {
        attempt,
        length: Array.isArray(res) ? res.length : "non-array",
      });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        if (!token) {
          applyList([], "ready", { source: "no-auth" });
          return;
        }
        ...
        if (!state.data.length)
          push({ type: "error", title: "Portfolios load failed", message: msg });
      }
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    if (!token) return; // skip refresh if not logged in
    const iv = setInterval(() => {
      fetchPortfolios()
        .then((list) => {
          if (list) applyList(list, undefined, { source: "interval" });
        })
        .catch(() => {});
    }, 30000);
    return () => clearInterval(iv);
  }, [fetchPortfolios, applyList, token]);

  // CRUD handlers
  async function createPortfolio(name: string) {
    if (!token) return; // disallow when not signed in
    ...
    try {
      ...
      applyList(
        dataRef.current.map((p) => (p.id === optimistic.id ? created : p)),
        undefined,
      ...
      applyList(
        dataRef.current.filter((p) => p.id !== optimistic.id),
        undefined,
        { action: "create:rollback", error: (e as Error)?.message },
      );
  ...
  async function renamePortfolio(id: string, name: string) {
    ...
    applyList(
      ...
      undefined,
      { action: "rename:optimistic", id },
    );
    try {
      const updated = await api.renamePortfolio(id, trimmed);
      applyList(
        prev.map((p) => (p.id === id ? updated : p)),
        undefined,
        { action: "rename:confirmed", id },
      );
    } catch (e) {
      applyList(prev, undefined, { action: "rename:rollback", id, error: (e as Error)?.message });
      push({ type: "error", title: "Rename failed", message: (e as Error)?.message || "Error" });
    }
  }
  async function deletePortfolio(p: Portfolio) {
    ...
    applyList(
      ...
      undefined,
      { action: "delete:optimistic", id: p.id },
    );
    try {
      await api.deletePortfolio(p.id);
      if (selectedId === p.id) setSelectedId(dataRef.current[0]?.id || null);
      debugLog("delete:confirmed", { id: p.id });
    } catch (e) {
      applyList(prev, undefined, {
        action: "delete:rollback",
        id: p.id,
        error: (e as Error)?.message,
      });
      push({ type: "error", title: "Delete failed", message: (e as Error)?.message || "Error" });
    }
  }
  ...
  useEffect(() => {
    if (showCreate) setTimeout(() => newInputRef.current?.focus(), 30);
  }, [showCreate]);
  ...
  function beginEdit(p: Portfolio) {
    setEditingId(p.id);
    setEditingVal(p.name);
  }
  function commitEdit() {
    if (editingId) {
      renamePortfolio(editingId, editingVal);
      setEditingId(null);
    }
  }

  // Drag and drop handlers for portfolio reordering
  const handleDragStart = (e: React.DragEvent, index: number) => {
    setDraggedIndex(index);
    e.dataTransfer.effectAllowed = "move";
    e.dataTransfer.setData("text/plain", portfolios[index].id);
  };
  ...
  const handleDrop = (e: React.DragEvent, dropIndex: number) => {
    ...
    const draggedPortfolio = newPortfolios[draggedIndex];

    // Remove from old position
    newPortfolios.splice(draggedIndex, 1);

    // Insert at new position
    newPortfolios.splice(dropIndex, 0, draggedPortfolio);

    // Update state and persist order
    applyList(newPortfolios, "ready", { source: "drag-drop-reorder" });
  ...
  function navigate(id: string) {
    const href = `/portfolios/${id}`;
    debugLog("navigate:click", { id, href });
    setSelectedId(id);
    let soft = true;
    try {
      router.push(href);
    ...
    setTimeout(() => {
      if (soft && window.location.pathname !== href) {
        debugLog("navigate:fallback-160", { id, href, current: window.location.pathname });
    ...
    setTimeout(() => {
      if (window.location.pathname !== href) {
        debugLog("navigate:force-hard-reload", { id, href, current: window.location.pathname });
  ...
  const portfolios = token ? state.data : [];
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        return (
          <div
            ...
            style={{
              ...
              opacity: isDragging ? 0.5 : isTemp ? 0.6 : 1,
              position: "relative",
              cursor: !editing && !isTemp ? (isDragging ? "grabbing" : "grab") : "default",
              transition: "opacity 0.2s ease",
            }}
            onMouseEnter={(e) => {
              const act = e.currentTarget.querySelector<HTMLDivElement>(".pf-actions");
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              >
                {token && !editing && (
                  <button title="Rename" onClick={() => beginEdit(p)} className="mini-btn">
                    ✎
                  </button>
                )}
                {token && (
      ...
      {token && showCreate && (
        <form onSubmit={submitCreate} style={{ display: "flex", gap: 4 }}>
          ...
          />
          <button type="submit" className="mini-btn" disabled={!newName.trim()}>
            Add
          </button>
          <button
            type="button"
      ...
      {!loading && portfolios.length === 0 && (
        <div style={{ fontSize: 12, opacity: 0.6 }}>No portfolios</div>
      )}
    </div>
  );
}
