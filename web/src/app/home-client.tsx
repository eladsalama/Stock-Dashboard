...
function OfflineBanner() {
  ...
  return (
    <div style={{ background: '#582', padding: '4px 8px', fontSize: 12, borderRadius: 4, marginBottom: 8 }}>
      Offline - cached data
    </div>
  );
}
      ...
      <section>
        ...
        <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
          {portfolios.map(p => {
            ...
            return (
              <li key={p.id} style={{ padding: '8px 0', display: 'grid', gridTemplateColumns: '1fr auto auto auto', gap: 12, alignItems: 'center', borderBottom: '1px solid #161b22' }}>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  {!isEdit && <Link href={`/portfolios/${p.id}`}>{p.name}</Link>}
                  {isEdit && (
                    <form onSubmit={(e) => { e.preventDefault(); commitEdit(p.id); }} style={{ display: 'flex', gap: 4 }}>
                      <input autoFocus value={editName} onChange={e => setEditName(e.target.value)} onBlur={() => commitEdit(p.id)} style={{ padding: '2px 4px', fontSize: 13, background: '#0d1117', color: '#e6edf3', border: '1px solid #30363d', borderRadius: 4 }} />
                    </form>
                  )}
                  <span style={{ fontSize: 11, opacity: 0.6 }}>Base: {p.baseCcy}</span>
                </div>
                <span style={{ fontSize: 11, opacity: 0.7 }}>{p.lastIngestStatus ?? 'no ingests'}</span>
                <span style={{ fontSize: 11, opacity: 0.5 }}>{ingestLabel}</span>
                <div style={{ display: 'flex', gap: 6 }}>
                  {!isEdit && <button onClick={() => startEdit(p)} style={iconBtnStyle}>Rename</button>}
                  <button onClick={() => remove(p.id)} style={{ ...iconBtnStyle, color: '#f85149' }}>Del</button>
                </div>
              </li>
            );
          })}
