...
export default function DashboardClient({ portfolio }: Props) {
  const [holdings, setHoldings] = useState<HoldingRow[]>(() => portfolio.positions.map(p => ({ id: p.id, symbol: p.symbol, quantity: Number(p.quantity), avgCost: Number(p.avgCost) })));
  const [selected, setSelected] = useState<string | null>(() => (typeof localStorage !== 'undefined' ? localStorage.getItem('dash.selectedSymbol') : null) || (holdings[0]?.symbol || null));
  const [range, setRange] = useState<RangeKey>(() => (typeof localStorage !== 'undefined' ? (localStorage.getItem('dash.range') as RangeKey) : null) || '1D');
      ...
      try {
        ...
        setHoldings(hs => hs.map(h => {
          ...
          if (!qq) return h;
            ...
            const dayChange = previousClose != null ? (price - previousClose) * h.quantity : undefined;
  ...
  const holdingColumns: Column<HoldingRow>[] = [
    ...
    { key:'price', label:'Price', sortable:true, align:'right', render: h => h.price ? h.price.toFixed(2) : '…' },
  ...
  return (
    <div style={{ display:'grid', gridTemplateColumns:'1fr 300px', gap:24 }}>
      <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
        <HeaderBar total={aggregateMV} dayChange={aggregateDayChange} dayPct={aggregateDayPct} lastQuotesAt={lastQuotesAt} loading={loadingQuotes} />
        <ChartCard symbol={selected} range={range} onRangeChange={setRange} candles={candles} loading={loadingChart} holdings={holdings} />
      </div>
      <div>
        <div className="panel" style={{ maxHeight:'70vh', overflow:'hidden', display:'flex', flexDirection:'column' }}>
          ...
          <div className="panel-body" style={{ padding:0, overflow:'auto' }}>
            <DataTable columns={holdingColumns} rows={holdings} rowKey={r=>r.id} emptyMessage="No holdings" />
          </div>
        </div>
      </div>
    </div>
...
function HeaderBar({ total, dayChange, dayPct, lastQuotesAt, loading }:{ total:number; dayChange:number; dayPct:number; lastQuotesAt: Date | null; loading:boolean }) {
  return (
    <div className="panel" style={{ padding:0 }}>
      <div className="panel-body" style={{ display:'flex', gap:32, flexWrap:'wrap' }}>
        ...
        <Metric label="Day Change" value={dayChange ? dayChange.toFixed(2) : '—'} className={dayChange>=0?'pl-pos':'pl-neg'} />
        <Metric label="Day %" value={dayChange ? dayPct.toFixed(2)+'%' : '—'} className={dayChange>=0?'pl-pos':'pl-neg'} />
...
function ChartCard({ symbol, range, onRangeChange, candles, loading, holdings }:{ symbol:string|null; range:RangeKey; onRangeChange:(r:RangeKey)=>void; candles:Array<{t:string;c:number}>; loading:boolean; holdings:HoldingRow[] }) {
    ...
    <div className="panel" style={{ minHeight:400, display:'flex', flexDirection:'column' }}>
      <div className="panel-header" style={{ gap:16, flexWrap:'wrap' }}>
        ...
        <div style={{ display:'flex', gap:6 }}>
          {RANGES.map(r => (
            <button key={r} onClick={() => onRangeChange(r)} className="inline-btn" style={r===range?{ background:'var(--color-bg-hover)', color:'var(--color-accent)' }: undefined}>{r}</button>
          ))}
        </div>
      </div>
...
function MiniLine({ data }:{ data:Array<{ t:string; c:number }> }) {
  ...
  return (
    <svg viewBox="0 0 100 100" preserveAspectRatio="none" style={{ position:'absolute', inset:0 }}>
      <polyline points={points} fill="none" stroke={up ? 'var(--color-success)' : 'var(--color-danger)'} strokeWidth={1.2} vectorEffect="non-scaling-stroke" />
    </svg>
  );
}
