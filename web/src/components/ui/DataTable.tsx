...
export function DataTable<T>({
  ...
  function toggle(col: Column<T>) {
    ...
    const current = sort;
    if (!current || current.key !== String(col.key))
      return onSortChange({ key: String(col.key), dir: "asc" });
  ...
  return (
    <table className="data-table" style={dense ? { fontSize: 11 } : undefined}>
      ...
      <tbody>
        ...
        {sorted.length === 0 && (
          <tr>
            <td colSpan={columns.length} style={{ padding: "6px 8px", opacity: 0.6 }}>
              {emptyMessage}
            </td>
          </tr>
        )}
