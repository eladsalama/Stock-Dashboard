...
const portfoliosRoutes: FastifyPluginAsync = async (app) => {
  ...
  const row = lines[i].split(',').map((c: string) => c.trim());
    ...
    await app.prisma.$transaction(async (tx) => {
      for (const r of out) {
        // @ts-ignore composite unique selector name
        await tx.position.upsert({
          // @ts-ignore composite unique selector generated by Prisma
          where: { portfolioId_symbol: { portfolioId: id, symbol: r.symbol } } as any,
