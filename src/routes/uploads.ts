...
const uploadsRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post("/v1/uploads/positions:presign", async (req, reply) => {
    ...
    const putParams: { Bucket: string; Key: string; ContentType: string; ChecksumCRC32?: string } =
      { Bucket: BUCKET, Key: key, ContentType: contentType };
    if (checksumCrc32) putParams.ChecksumCRC32 = checksumCrc32;
    const cmd = new PutObjectCommand(putParams);
    const url = await getSignedUrl(s3, cmd, { expiresIn: 60 * 5 });
    req.log.info(
      { key, urlSnippet: url.split("?")[0], bucket: BUCKET },
      "generated positions presign",
    );
    return {
      bucket: BUCKET,
      key,
  ...
  app.post("/v1/uploads/enqueue", async (req, reply) => {
    ...
    try {
      ...
      try {
        ...
        req.log.error(
          { err: e, portfolioId, key },
          "failed to insert pending ingest run (continuing)",
        );
      }
      await sqs.send(
        new SendMessageCommand({
          QueueUrl: QUEUE_URL,
          MessageBody: JSON.stringify({ portfolioId, key }),
        }),
      );
