...
const quotesRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/v1/search', { config: { rateLimit: { max: 30, timeWindow: '1 minute' } } }, async (req, reply) => {
    ...
    try {
      ...
      const items = (res.quotes || [])
        ...
        .slice(0, 10)
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        .map((r: any) => ({ symbol: r.symbol, shortname: r.shortname, longname: r.longname || r.shortname, exch: r.exchange || r.exch }));
      return { items };
    } catch (e) {
