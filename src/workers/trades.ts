...
async function positionsSnapshotIngest(
  portfolioId: string,
  key: string,
  prisma: PrismaClient,
  s3KeyFetcher: () => Promise<string>,
) {
  ...
  const lines = text
    ...
    .map((l) => l.trim())
    .filter(Boolean);
  console.log("[worker][positions] line count after trim/filter", {
    key,
    lines: lines.length,
    sample: lines.slice(0, 3),
  });
  if (!lines.length) {
    console.warn("[worker][positions] empty file", { key, portfolioId });
  ...
  const looksLikeHeader =
    ...
    (headerLower.includes("quantity") || headerLower.includes("qty")) &&
    (headerLower.includes("avgcost") ||
      headerLower.includes("avg_cost") ||
  ...
  try {
    const existing = (await prisma.$queryRawUnsafe(
      `SELECT id FROM "IngestRun" WHERE "portfolioId" = $1 AND "objectKey" = $2 ORDER BY "startedAt" DESC LIMIT 1`,
      portfolioId,
      key,
    )) as Array<{ id: string }>;
  ...
  await prisma.$transaction(async (tx: TxClient) => {
    for (let i = startIndex; i < lines.length; i++) {
      ...
      if (!symbol || !isFinite(quantity) || !isFinite(avgCost)) {
        failed++;
        continue;
      }
    ...
    await tx.portfolio.update({
      ...
      data: { lastIngestAt: new Date(), lastIngestStatus: `positions:${inserted}` },
    });
  });
  await prisma.$executeRawUnsafe(
    ...
    while (true) {
      ...
      for (const m of messages) {
        try {
          ...
          console.log(
            `[worker] message parsed portfolio=${portfolioId || "N/A"} key='${key}' attempt=${currentAttempt}`,
          );
          if (portfolioId && key) {
            try {
              ...
              if (isPositions) {
                console.log(
                  `[worker] positions snapshot ingest start portfolio=${portfolioId} key='${key}'`,
                );
                const s3 = getS3();
                const obj = await s3.send(
                  new GetObjectCommand({
                    Bucket: process.env.TRADES_BUCKET || "trades-bucket",
                    Key: key,
                  }),
                );
                const textLoader = async () => (await obj.Body?.transformToString()) || "";
                const { inserted, runId } = await positionsSnapshotIngest(
                  portfolioId,
                  key,
                  prisma,
                  textLoader,
                ...
                console.log(
                  `[worker] positions snapshot ingest done portfolio=${portfolioId} key='${key}' runId=${runId} rows=${inserted}`,
                );
              } else {
                ...
                console.log(
                  `[worker] trade ingest done portfolio=${portfolioId} key='${key}' inserted=${result.inserted}`,
                );
              }
            } catch (err) {
              ...
              if (attempt >= MAX_ATTEMPTS) {
                await sqs.send(
                  new SendMessageCommand({
                    QueueUrl: DLQ_URL,
                    MessageBody: JSON.stringify({ portfolioId, key, error: String(err) }),
                  }),
                );
                console.error(`[worker] moved to DLQ after ${attempt} attempts`, {
                  key,
                  portfolioId,
                  error: String(err),
                });
                ...
                await sqs.send(
                  new SendMessageCommand({
                    QueueUrl: QUEUE_URL,
                    MessageBody: JSON.stringify({ portfolioId, key, attempt }),
                    DelaySeconds: delay,
                  }),
                );
                console.warn(`[worker] enqueue retry #${attempt} with delay=${delay}s`, {
                  key,
                  portfolioId,
                });
              }
