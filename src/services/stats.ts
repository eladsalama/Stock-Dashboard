...
export interface SymbolStats {
  ...
  forwardDividendYield?: number; // yield fraction (0.023)
...
export async function fetchYahooStats(symbol: string): Promise<SymbolStats> {
  ...
  const earnings =
    cal.earnings || cal.earningsDate || cal.earnings ? cal.earningsDate || cal.earnings : undefined;
  const poss =
    earnings && Array.isArray(earnings)
      ? earnings
      : Array.isArray(cal.earningsDate?.raw)
        ? cal.earningsDate.raw
        : undefined;
  if (Array.isArray(poss) && poss.length) {
    ...
    if (typeof first === "number") {
      ...
      if (typeof raw === "number")
        earningsDate = new Date(raw * (raw < 1e12 ? 1000 : 1)).toISOString();
    }
  ...
  if (summaryDetail?.exDividendDate?.raw) {
    ...
    if (typeof raw === "number")
      exDividendDate = new Date(raw * (raw < 1e12 ? 1000 : 1)).toISOString();
  }
  let fiscalYearEnd: string | undefined;
  if (keyStats?.lastFiscalYearEnd?.raw) {
    const raw = keyStats.lastFiscalYearEnd.raw;
    if (typeof raw === "number")
      fiscalYearEnd = new Date(raw * (raw < 1e12 ? 1000 : 1)).toISOString();
  }
  ...
  return {
    ...
    bid: numberOrU(price.bid),
    bidSize: numberOrU(price.bidSize),
    ask: numberOrU(price.ask),
    askSize: numberOrU(price.askSize),
    dayLow: numberOrU(price.regularMarketDayLow),
    dayHigh: numberOrU(price.regularMarketDayHigh),
    fiftyTwoWeekLow: numberOrU(summaryDetail.fiftyTwoWeekLow),
    fiftyTwoWeekHigh: numberOrU(summaryDetail.fiftyTwoWeekHigh),
    volume: numberOrU(price.regularMarketVolume ?? summaryDetail.volume),
    avgVolume: numberOrU(
      summaryDetail.averageDailyVolume3Month ?? summaryDetail.averageDailyVolume10Day,
    ),
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function numberOrU(v: any): number | undefined {
  const n = Number(v);
