...
export function parsePositionsSnapshot(
  csv: string,
): Array<{ symbol: string; quantity: number; avgCost: number }> {
  const lines = csv
    .split(/\r?\n/)
    .map((l) => l.trim())
    .filter((l) => l.length);
...
export async function parsePositionsCsvAndUpsert(
  portfolioId: string,
  key: string,
  csvText: string,
  prisma: PrismaClient,
  ...
  await prisma.$transaction(async (tx: TxClient) => {
    for (const r of rows) {
      ...
      if (existing) {
        ...
        await tx.position.create({
          data: { portfolioId, symbol: r.symbol, quantity: r.quantity, avgCost: r.avgCost },
        });
      }
      ok++;
    }
    await tx.portfolio.update({
      ...
      data: { lastIngestAt: new Date(), lastIngestStatus: `positions:${ok}` },
    });
  });
  await prisma.$executeRawUnsafe(
